    ...
    return (
      <div className="flex items-center gap-2">
        <div className="flex-1 h-4 bg-muted rounded-sm overflow-hidden relative min-w-[120px]">
          ...
          <span className="absolute inset-0 text-[10px] font-mono text-foreground flex items-center justify-center">
            {Math.round(level)}L / {Math.round(capacity)}L
          </span>
        </div>
    ...
    <>
      <AppHeader title="Fuel">
        <Button variant="outline" size="sm" className="mr-2">
          <Droplets className="h-4 w-4 mr-1" /> Record Delivery
        </Button>
        <Button size="sm" className="bg-accent text-accent-foreground hover:bg-accent/90">
          <Plus className="h-4 w-4 mr-1" /> Issue Fuel
        </Button>
      </AppHeader>
      <div className="flex-1 overflow-auto p-6">
        <div className="bg-card border border-border">
          <div className="p-4 border-b border-border">
            <h2 className="text-sm font-semibold">Fleet Fuel Overview</h2>
          </div>
          <div className="divide-y divide-border">
            {loading ? (
              <div className="p-8 text-center text-muted-foreground text-sm">Loading...</div>
            ) : trucks.length === 0 ? (
              <div className="p-8 text-center text-muted-foreground text-sm">No trucks registered.</div>
            ) : (
              trucks.map((truck, i) => (
                <motion.div
                  key={truck.id}
                ...
                >
                  ...
                  <div className="flex-1 max-w-md">
                    {fuelBar(Number(truck.estimated_fuel_level_litres), Number(truck.fuel_tank_capacity_litres))}
                  </div>
                </motion.div>
              ))
            )}
