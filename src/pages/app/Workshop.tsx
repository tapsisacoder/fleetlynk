...
const Workshop = () => {
  ...
  const [prodJobCards, setProdJobCards] = useState<any[]>([]);
  ...
  const jobCards = demo
    ? demo.jobCards.map(jc => ({ ...jc, trucks: { registration_number: jc.truck_reg } }))
    : prodJobCards;

  const filtered = jobCards.filter(jc =>
    jc.job_card_number?.toLowerCase().includes(search.toLowerCase()) ||
    jc.trucks?.registration_number?.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                : filtered.map((jc, i) => (
                  <motion.tr key={jc.id || i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
      ...
      {!demo && <NewJobCardDialog open={showNew} onOpenChange={setShowNew} onCreated={() => {}} />}
    </>
  );
};
